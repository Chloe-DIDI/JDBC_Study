...
//					--------- --------
	...
	public static Connection getConnection()
	{
		...
		{
			try
			{	// ▩지정한 url ~ 패스워드를 넘겨주겟다!	 → 잘되는지확인 Test02 가서 함     
				String url = "jdbc:oracle:thin:@localhost:1521:xe";
				//-- localhost 는 오라클 서버의 ip 주소를 기재하는 부분
				//   1521 은 오라클 포트 넘버(Port Number)
				//   xe 는 오라클 SID(Express Edition 은 xe)
				//-- 오라클 리스너 → 포트넘버(1521)여기에 가고싶어하는 사람을 
				//		  	   		  여기로 데려다주는 역할을 하는게 리스너(클럽삐끼)다~!
...
//							-------------  
	...
	public static void close()
	{
		...
		if (dbConn != null)
		{
			try
			{
				// 연결 갹체릐 isClosed() 메소드를 통해 연결 상태 확인
				//-- 연결이 닫혀있는 경우 true 반환
