...
---------------------------------------
/*
LOC 로 입력해서 TBL_CITY 에 LOC를 NAME 으로 바꾸려고하면
이미 TBL_EMP 까지 작성까지 다 완성한 상태이기 때문에
PK, FK 관계성 때문에 TBL_CITY만 삭제안되고 데이터도 삭제안된다
이럴때에 그냥 역순으로 다 하나씩 삭제하고 
다시 생성하면됨
...
FROM TBL_JIKWI;
--> 한 줄 구성
SELECT MIN_BASICPAY FROM TBL_JIKWI
;

--⑤ 데이터(직원 정보) 입력 쿼리문
INSERT INTO TBL_EMP(EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG)
VALUES (EMPSEQ.NEXTVAL, '김진희', '990320-2234567', TO_DATE('2019-07-14', 'YYYY-MM-DD')
...
SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_NAME, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, E.BASICPAY, E.SUDANG, (E.BASICPAY+E.SUDANG) AS SAL FROM TBL_EMP E LEFT JOIN TBL_BUSEO B
...
--10. 직원 정보 수정 쿼리문 구성  
UPDATE TBL_EMP
SET EMP_NAME='가가가', SSN='900105-252526'
  , IBSADATE='2020-11-11'
  , CITY_ID=1
  , TEL='010-1111-9225'
  , BUSEO_ID=1
...
WHERE EMP_ID = 1061;
--> 한 줄 구성
UPDATE TBL_EMP SET EMP_NAME='가가가', SSN='900105-252526', IBSADATE='2020-11-11', CITY_ID=1, TEL='010-1111-9225', BUSEO_ID=1, JIKWI_ID=1, BASICPAY=2500000, SUDANG=250000 WHERE EMP_ID = 1061
;
...
;
	...
	city_name, buseo_name, jikwi_name, emp_name, ssn, ibsadate, tel; 
city_id, buseo_id, jikwi_id, min_basicpay, emp_id, basicpay, sudang;
