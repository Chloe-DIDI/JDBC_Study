...
public class Process
{
	...
	// Scanner인스턴스 생성
	// do~while문 구성 『(".")』하면 끝난다
	// 사용자에게 국어 영어 수학을 입력 하려는 것은 insert ->add메소드 호출 ===>>>dto구성
	// Score DTO 객체 구성
	// dao의 add()메소드 호출
	// 데이터 베이스 연결 종료
	...
	// 1. try~catch작성 안에서
	// 2. dao의 connection() 메소드 호출 ==> 데이터 베이스 연결
	// 3. dao의 count() 메소드 호출 ==> 인원수 확인 ==>> data 바인딩
	...
	public void sungjukSearchName()
	{
		try
		{
			...
			//==>> 데이터베이스를 가져오기전에 숫자형태로 입력하면 안되기때문에 유효성검사를 하는것이다! 
	...
	public void sungjukUpdate()
	{
		try
		{
		    ...
		    if (arrayList.size() > 0)
			{
				...
				dto.setEng(eng);
				dto.setMat(mat);
				dto.setSid(String.valueOf(sid));//String 클래스에 값이 있음??
